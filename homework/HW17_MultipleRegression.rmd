...
Turnout seems to correlate well with white percentage and has a negative correlation with unemployment. Income and bachelor's or higher also have a strong correlation. Median age and bachelor's or higher have no correlation. Income and percent white have the next lowest correlation. Based on the scatterplots voter turnout seems to have a linear relationship with unemployment and slightly with percent white. Median age doesn't seem to have a linear relationship.
...
With an adjusted R-squared of 0.8023, a high F-statistic, and low p-value we can say that the multiple variable model is a good predictor for voter turnout. The values that seems to be insignificant are population, unemployment, and bachelor's or higher. The model seems to be useful because it of the statistics mentioned earlier and with a large number of it's variables contributing to overall turnout. 
...
The model I was able to make with the highest r-squared model included income, median age, and pct_white. This could be because the values don't have strong direct relations with one another.
...
# Model 3
model3 <- lm(turnout ~ income + pct_bachelors_higher + pct_unemployed, data = md_turnout)
summary(model3)

# Model 4
model4 <- lm(turnout ~ population + pct_unemployed + pct_bachelors_higher, data = md_turnout)
summary(model4)
...
You could use this analysis to cover an election/voter participation story by curating the model to focus on factors that contribute to voter turnout and what measures counties are taking to boost these numbers. If a decade has passed since a county passes a bill that encouraged attainment of a bachelor's degree right out of high school, we could use a model to see the relation between age, bachelor's attainment, and voter turnout. This would give us a glimpse into how the bill may have helped or harmed voter turnout.
