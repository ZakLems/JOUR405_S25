...
songs |> 
  ggplot() + 
  geom_histogram(aes(x = danceability), binwidth = 0.07) 
```
...
The distribution of song danceabilty is right skewed. The standard deviation shows that 68% of the songs in the dataset have a danceability rating between 0.51 and 0.83. A good story could possibly look at the correlation between overall popularity and danceability. The histogram does show that songs skew towards being more danceable overall but does that coincide with overall streaming numbers or is it just an out of touch industry trend. Also looking at previous years to see if the data has been gradually skewing any direction overtime could be interesting.
