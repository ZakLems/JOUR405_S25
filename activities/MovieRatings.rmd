...
The observed data and expected data are pretty similar. The biggest distance in expected and observed is with five star reviews by 15 reviews, which isn't that much for a sample size of 400. 
...
The right tail critical value is less than the calculated chi-square statistic, meaning we reject the null hypothesis and the observed frequencies are significantly different from the expected frequencies or the ratings are not consistent with the expected percentages and we should dig a bit deeper to see why.
